...
JSON;
  ...
  public function getUserMetrics(array $filters) {
    ...
    if (count($userRecordingData->aggregations->user_limit->by_user->buckets) > 0) {
      ...
      $firstInSeasonRecordDateArray = $this->getFirstInSeasonDateArray(
        $userInfo->first_record_date->value_as_string
      );
      $lastInSeasonRecordDateArray = $this->getLastInSeasonDateArray(
        $userInfo->last_record_date->value_as_string
      );
      $inSeasonRecordingDaysTotal = $this->countInSeasonDaysBetween(
      ...
      if ($inSeasonRecordingDaysTotal === 0) {
        ...
        $activityRatio = round(
          (100 * $inSeasonRecordingDaysActive) / $inSeasonRecordingDaysTotal,
          1
        );
      }
  ...
  public function getCounts(array $filters, array $categories) {
    ...
    if (in_array('species', $categories)) {
      $aggs['species_count'] = [
        'cardinality' => ['field' => 'taxon.species_taxon_id'],
      ];
    }
    ...
    if (in_array('recorders', $categories)) {
      $aggs['recorder_count'] = [
        'cardinality' => ['field' => 'event.recorded_by.keyword'],
      ];
    }
...
JSON;
    ...
    foreach ($response->aggregations->taxa->buckets as $taxon) {
      ...
      $fieldValues['total_individual_count'] =
        $taxon->total_individual_count->value;
   ...
   */
  private function countInSeasonDaysBetween(
    array $firstInSeasonRecordDateArray,
    array $lastInSeasonRecordDateArray
  ) {
    // Total number of season days in the years the user has been recording.
    $inSeasonRecordingDaysTotal =
      92 *
      ($lastInSeasonRecordDateArray['year'] -
        $firstInSeasonRecordDateArray['year'] +
        1);
  ...
  private function getEsResponse($request) {
    ...
    curl_setopt(
      $session,
      CURLOPT_HTTPHEADER,
      ElasticsearchProxyHelper::getHttpRequestHeaders($config)
    );
...
JSON;
    ...
    // ES 6/7 tolerance.
    $this->projectRecordsCount = isset($taxaResponse->hits->total->value)
      ? $taxaResponse->hits->total->value
      : $taxaResponse->hits->total;
    $this->projectSpeciesCount = count(
      $taxaResponse->aggregations->species_list->buckets
    );
  ...
  private function getSpeciesWithRarity() {
    ...
    foreach ($speciesList as $i => $speciesInfo) {
      ...
      if (
        $recordsFoundSoFar > $this->projectRecordsCount / 2 &&
        !$this->medianOverallRarity
      ) {
...
JSON;
    $response = $this->getEsResponse($request);
    // ES 6/7 tolerance.
    return isset($response->hits->total->value)
